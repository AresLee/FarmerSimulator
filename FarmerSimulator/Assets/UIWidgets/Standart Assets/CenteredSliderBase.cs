...
namespace UIWidgets
{
		...
		where T : struct
	{
		...
		protected float RangeSize()
		{
			if (IsHorizontal())
			{
				return UsableRangeRect.rect.width - (HandleSize() / 2);
			}
			else
			{
				return UsableRangeRect.rect.height - (HandleSize() / 2);
			}
		}
		...
		protected float GetStartPoint()
		{
			...
			var result = (IsHorizontal())
				? pos.x - (rect.width * pivot.x)
				: pos.y - (rect.height * pivot.y);
			return result;
		}
		...
		{
			var new_position = HandleRect.position;
			if (IsHorizontal())
			{
				new_position.x = ValueToPosition(_value);
			}
			else
			{
				new_position.y = ValueToPosition(_value);
			}
			HandleRect.position = new_position;
