...
namespace UIWidgets
{
	...
	public class RangeSliderHandle : Selectable, IDragHandler, IInitializePotentialDragHandler, ISubmitHandler
	{
		...
		public virtual void OnDrag(PointerEventData eventData)
		{
			...
			curCursor -= RectTransform.rect.position;

			Vector2 limits = PositionLimits();

			Vector3 new_position = RectTransform.position;
			if (IsHorizontal())
			{
			...
			{
				RectTransform.position = new_position;
		...
		bool IsNewPosition(Vector3 pos)
		{
			if (IsHorizontal())
			{
				return RectTransform.position.x!=pos.x;
			}
			else
			{
				return RectTransform.position.y!=pos.y;
			}
		}
